...
const SKIP_DIRS = new Set([
  ...
  '.idea', '.vscode', '.turbo', '.vercel', '.netlify',
]);
...
export async function detectStructure(projectRoot: string): Promise<StructureInfo> {
  ...
  folders.sort((a, b) => {
    const importantDirs = ['src', 'app', 'lib', 'packages', 'apps', 'api', 'pages', 'components'];
...
async function analyzeFolderShallow(folderPath: string, folderName: string): Promise<FolderInfo> {
  let fileCount = 0;
  ...
  // Get description
  const description = FOLDER_PURPOSES[folderName.toLowerCase()] || inferFolderPurpose(folderName);
...
function inferFolderPurpose(name: string): string {
  ...
  if (lower.includes('asset') || lower.includes('static') || lower.includes('public')) return 'Static assets';
  ...
  if (lower.includes('guard') || lower.includes('auth')) return 'Authentication/Authorization';
